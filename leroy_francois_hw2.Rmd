...
for(iii in 1:length(alphas)){
  ...
  for(ii in 1:length(grid)){
    ...
    for(i in 1:10){
      ...
      pred <- predict(model, newx = x.test, type = "response")
      p <- prediction(pred, cv_test_data$Purchase)
      ## Compute auc
      auc <- performance(p, measure="auc", fpr.stop = .2)
      auc <- auc@y.values[[1]]
...
3. A regularized logistic regression with $\lambda = 0.01$ and $\alpha = 0.75$
...
## Predict with the model
pred <- predict(opti_glm, newx = x.test, type = "response")
p <- prediction(pred, d_test$Purchase)
